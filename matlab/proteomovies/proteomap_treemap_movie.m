function proteomap_treemap_movie(organism,data_directory,movie_name,options)

% proteomap_treempa_movie(organism, data_directory, movie_name, options)
%
...
options_def.output_directory    = '~/projekte/protein_abundance/ps-files/rectangular_proteomaps/movies';
...
options_def.interpolation_steps = 50; % # of interpolation intervals = # of frames - 1
...
movie_save(sprintf('proteomap_movie_%s_lv%d', strrep(movie_name,' ','_'), options.show_level), M,my_delay);
display(sprintf('in directory %s',options.output_directory));
