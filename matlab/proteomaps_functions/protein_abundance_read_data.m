function [data_sets] = protein_abundance_read_data(data_directory,protein_lengths,n_randomised_trees,pathnames)

cd(pathnames.PROTEOMAPS_MATLAB_DIR);
...
if strcmp(data_directory(end),'/'), data_directory = data_directory(1:end-1); end
...
data_filenames   = A(:,2)';
data_shortnames  = A(:,3)';
data_shortnames = strrep(data_shortnames,'.','_');
...
protein_data = protein_abundance_read_data_once(data_organisms, data_shortnames, data_filenames, data_legendnames, data, ko_list,data_directory, verbose);
...
for itt = 1:n_randomised_trees,
  if verbose, display(sprintf('Permutation sample %d/%d',itt,n_randomised_trees)); end
  my_ko_tree = load_file_as_strings(sprintf([pathnames.SUBSAMPLED_TREE_DIR '/KO_hierarchy_standardised_multiple_occurance_%d.tms'],itt));
...
cd(pathnames.PROTEOMAPS_MATLAB_DATA_DIR);
