...
% proteomap_draw_treemap(ko_tree, cumulative_values, data_sets_short, options)
%
% Draw a rectangle treemap based on a KO number hierarchy (proteomap)
%
% ko_tree           list of strings describing the functional hierarchy
...
%                              instead of a filename, this argument can also directly contain a color matrix 
%                              (referring to all entries in ko_tree)
% options.output_directory   = directory for picture files
% options.show_level         = 2; level for category labels
% options.window_position    = [120,20,1000,1000]; 
% options.fignums            = []; 
...
default_options.window_position    = [120,20,1000,1000]; 
default_options.fignums            = []; 
...
if sum(isfinite(cumulative_values))==0,
  clf; return;
end

if isempty(options.fignums),
  options.fignums = 1:length(data_sets_short);
end
% -----------------------------------------------
% initialise
...
if isnumeric(options.color_file),
  colortable.names  = ko_tree;
  colortable.colors = options.color_file;
else
...
for itt = 1:length(data_sets_short),
  ...
  rects = treemap_hierarchical(options.rect,names_c,summed_sizes_c,colors_c,levels_c,split_direction,options.fixed_arrangement);
    ...
    %% graphics 
    figure(options.fignums(itt)); set(gcf,'Position',options.window_position); clf;
    names_c = strrep(names_c,'_',' ');
    treemap_plot(levels_c,names_c,rects,colors_c,names_c,options);
